...
vec3 wind(vec2 input)
{
    const vec2 dir = vec2(1, 2);
    const vec2 dir2 = vec2(0.5, -1);
    float noise = perlin(input + dir*0.5*time);
    float noiseb = perlin(-input + dir2*0.5*time);
...
void main()
{
    ...
    vec3 offset = (gl_VertexID == 0) ? -billboard[0].xyz * spacing: (gl_VertexID == 1) ? billboard[0].xyz * spacing: (vec3(0,5,0)+w) * spacing;
